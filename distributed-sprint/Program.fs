...
let main argv =
    ...
    let mapDiff ([|dateS:string; openP:string; highP:string; LowP:string; closeP:string; s:string;a:string;|]) =
        let diff = abs ((Double.Parse openP) - (Double.Parse closeP))
        let result =  (dateS, diff) 
        result
    

    let result = List.map (splitCommas>>mapDiff) stockData.Tail 
                        |>List.maxBy snd
                        |> fst
    ...
    decideOn chronoTrigger |>printf "%A"
    printfn "%A" result
    decideOn halo |>printf "%A"
